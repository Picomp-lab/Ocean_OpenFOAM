"""
GT vs Prediction animation — vertically stacked.
Top: Ground Truth, Bottom: Prediction.

Usage:
    python -u compare_animation.py \
...
def rollout(model, coords, data, stats, window, start_frame, n_steps, device):
    ...
    fields_w = np.concatenate(
        ...
        axis=-1)
    fields_w = torch.from_numpy(fields_w).unsqueeze(0).to(device)
    coords_t = torch.from_numpy(coords).unsqueeze(0).to(device)

    for step in range(n_steps):
        inp = torch.cat([coords_t, fields_w], dim=-1)
        pred = model(inp)
...
def main():
    ...
    preds = rollout(model, coords, data, stats, W, start, n_steps, device)
    ...
    for ax, label in [(ax_gt, "Ground Truth"), (ax_pred, "Prediction")]:
