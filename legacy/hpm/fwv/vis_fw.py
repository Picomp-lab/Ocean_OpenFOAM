...
def main():
    ...
    ap.add_argument("--n_frames", type=int, default=0,
                    help="rollout 步数; 0 (或 <=0) = 从 start_frame 跑到 chunk 末尾 (全程)")
    ...
    # n_frames<=0 -> 跑到 chunk 末尾 (全程); >0 -> 取 min, 不越界
    n_steps = (T - start) if args.n_frames <= 0 else min(args.n_frames, T - start)
    assert n_steps > 0, f"start={start} 超出 chunk 长度 T={T}"
    print(f"n_frames={args.n_frames} -> n_steps={n_steps}  (chunk T={T}, start={start})")
